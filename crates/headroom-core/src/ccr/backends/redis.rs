...
use crate::ccr::{CcrMetadata, CcrStore};
...
impl CcrStore for RedisCcrStore {
    fn put_with_metadata(&self, hash: &str, payload: &str, original_tokens: usize, compressed_tokens: usize) {
        ...
        // Store as JSON envelope so metadata travels with the payload.
        ...
        let res: redis::RedisResult<()> =
            conn.set_ex(&key, envelope.to_string().as_bytes(), self.default_ttl_seconds);
        if let Err(err) = res {
            tracing::warn!(
                target = "ccr.redis",
                hash = %hash,
    ...
    fn get(&self, hash: &str) -> Option<String> {
        ...
        match bytes {
            Ok(Some(bytes)) => {
                // Try JSON envelope first, fall back to raw bytes (legacy).
                let s = String::from_utf8(bytes).ok()?;
                if let Ok(envelope) = serde_json::from_str::<serde_json::Value>(&s) {
                    envelope.get("payload").and_then(|p| p.as_str().map(String::from))
                } else {
                    Some(s)
                }
            }
    ...
    fn get_metadata(&self, hash: &str) -> Option<CcrMetadata> {
        ...
        let bytes: Option<Vec<u8>> = conn.get(&key).ok()?;
        let s = String::from_utf8(bytes).ok()?;
        let envelope: serde_json::Value = serde_json::from_str(&s).ok()?;
        Some(CcrMetadata {
            original_tokens: envelope["original_tokens"].as_u64().unwrap_or(0) as usize,
            compressed_tokens: envelope["compressed_tokens"].as_u64().unwrap_or(0) as usize,
        })
    }
