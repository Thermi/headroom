...
class TestSessionTrackerStore:
    ...
    def test_max_sessions_evicts_oldest(self):
        """When max_sessions is exceeded, oldest trackers are evicted."""
        ...
        # Adding a 6th should evict the oldest (session-0)
        store.get_or_create("session-6", "anthropic")
        assert "session-0" not in store._trackers
        assert store.active_sessions == 5

    def test_max_sessions_evicts_expired_first(self):
        """Expired sessions are evicted before active ones when over capacity."""
        import time

        store = SessionTrackerStore(max_sessions=3)
        store.get_or_create("session-a", "anthropic")
        store.get_or_create("session-b", "anthropic")
    ...
    def test_max_sessions_zero_means_unlimited(self):
        """max_sessions=0 disables eviction."""
        store = SessionTrackerStore(max_sessions=0)
        for i in range(100):
            store.get_or_create(f"session-{i}", "anthropic")
        assert store.active_sessions == 100
