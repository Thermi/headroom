...
class TestEnvInjection:
    ...
    def test_sets_both_env_vars(self, runner, tmp_path, monkeypatch) -> None:
        captured, _ = _capture_launch(
            runner, ["--port", "8787", "--no-rtk"], monkeypatch, tmp_path
        )
        env = captured["env"]
    ...
    def test_config_content_has_headroom_provider(
        self, runner, tmp_path, monkeypatch
    ) -> None:
        captured, _ = _capture_launch(
            runner, ["--port", "9999", "--no-rtk"], monkeypatch, tmp_path
        )
        env = captured["env"]
    ...
    def test_config_content_has_all_models(
        self, runner, tmp_path, monkeypatch
    ) -> None:
        captured, _ = _capture_launch(
            runner, ["--no-rtk"], monkeypatch, tmp_path
        )
        env = captured["env"]
        config = json.loads(env["OPENCODE_CONFIG_CONTENT"])
    ...
    def test_tool_label_and_agent_type(
        self, runner, tmp_path, monkeypatch
    ) -> None:
        captured, _ = _capture_launch(
            runner, ["--no-rtk"], monkeypatch, tmp_path
...
class TestExtraArgs:
    def test_passes_through_model_arg(
        self, runner, tmp_path, monkeypatch
    ) -> None:
        captured, _ = _capture_launch(
            runner,
...
class TestFlagForwarding:
    def test_custom_port_propagates(self, runner, tmp_path, monkeypatch) -> None:
        captured, _ = _capture_launch(
            runner, ["--port", "1234", "--no-rtk"], monkeypatch, tmp_path
        )
    ...
    def test_verbose_flag(self, runner, tmp_path, monkeypatch) -> None:
        captured, result = _capture_launch(
            runner, ["--verbose", "--no-rtk"], monkeypatch, tmp_path
        )
    ...
    def test_injects_rtk_instructions_when_rtk_present(
        ...
        with patch.object(wrap_mod.shutil, "which", return_value="opencode"):
            with patch.object(
                ...
                with patch.object(
                    wrap_mod, "_inject_rtk_instructions", side_effect=track_inject
                ):
...
class TestLeanCtxContextTool:
    def test_uses_lean_ctx_when_configured(
        self, runner, tmp_path, monkeypatch
    ) -> None:
        ...
        with patch.object(wrap_mod.shutil, "which", return_value="opencode"):
            with patch.object(
                wrap_mod, "_setup_lean_ctx_agent"
            ) as mock_lean_ctx:
                with patch.object(wrap_mod, "_launch_tool"):
                    result = runner.invoke(main, ["wrap", "opencode"])
