...
- Proxy mode: ``rtk_wiring="disabled"`` in ``SubscriptionTracker.__init__``
  and ``configure_subscription_tracker`` ensures RTK is skipped in proxy
  context since it has no purpose there. Tests 13-18 cover this.
"""
...
def test_rtk_wiring_ctor_disabled_skips_poll(monkeypatch: pytest.MonkeyPatch) -> None:
    """``rtk_wiring="disabled"`` in the constructor skips RTK polling entirely.

    This is the mechanism the proxy uses to disable RTK at startup. The poll
    short-circuits before calling ``_get_rtk_stats``.
    """
    tracker = _build_tracker(monkeypatch)
    monkeypatch.delenv(tracker_module._RTK_WIRING_ENV, raising=False)
    ...
    tracker._rtk_wiring = "disabled"
    tracker.update_contribution()

    assert polls[0] == 0, "stats endpoint must not be called when wiring is disabled"
    assert tracker._state.contribution.tokens_saved_rtk == 0
    assert tracker._last_rtk_tokens_saved == 0
...
def test_rtk_wiring_ctor_none_falls_back_to_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """``rtk_wiring=None`` (default) reads ``HEADROOM_RTK_WIRING`` env var.

    When the env var is ``disabled`` the poll is skipped. This ensures the
    backward-compatible path works for non-proxy callers.
    """
    tracker = _build_tracker(monkeypatch)
    monkeypatch.setenv(tracker_module._RTK_WIRING_ENV, "disabled")
...
def test_rtk_wiring_ctor_explicit_enabled_overrides_env(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """``rtk_wiring="enabled"`` takes precedence over env var ``disabled``."""
    tracker = _build_tracker(monkeypatch)
    monkeypatch.setenv(tracker_module._RTK_WIRING_ENV, "disabled")

    polls = _stub_rtk_stats(monkeypatch, [_session_payload(tokens_saved=42)])

    tracker._rtk_wiring = "enabled"
    tracker.update_contribution()

    assert polls[0] == 1, "stats endpoint must be called when wiring is enabled"
    assert tracker._state.contribution.tokens_saved_rtk == 42
...
def test_configure_subscription_tracker_passes_rtk_wiring(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """``configure_subscription_tracker(rtk_wiring="disabled")`` creates a tracker
    that skips RTK polling.
    """
    monkeypatch.setattr(SubscriptionTracker, "_load_persisted_state", lambda self: None)
    monkeypatch.setattr(SubscriptionTracker, "_try_acquire_rtk_poll_lock", lambda self: True)
    monkeypatch.delenv(tracker_module._RTK_WIRING_ENV, raising=False)
    ...
    tracker = tracker_module.configure_subscription_tracker(
        enabled=True,
        rtk_wiring="disabled",
    )

    assert tracker._rtk_wiring == "disabled"
...
def test_configure_subscription_tracker_default_rtk_wiring_is_none(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Default call without ``rtk_wiring`` leaves it ``None``, falling back to env."""
    monkeypatch.setattr(SubscriptionTracker, "_load_persisted_state", lambda self: None)
    monkeypatch.setattr(SubscriptionTracker, "_try_acquire_rtk_poll_lock", lambda self: True)
    monkeypatch.delenv(tracker_module._RTK_WIRING_ENV, raising=False)
    monkeypatch.setattr(tracker_module, "_tracker_instance", None)

    polls = _stub_rtk_stats(monkeypatch, [_session_payload(tokens_saved=777)])

    tracker = tracker_module.configure_subscription_tracker(enabled=True)

    assert tracker._rtk_wiring is None

    tracker.update_contribution()

    assert polls[0] == 1, "default None must fall through to env (enabled) and poll"
    assert tracker._state.contribution.tokens_saved_rtk == 777
...
@pytest.mark.asyncio
async def test_proxy_creates_tracker_with_rtk_disabled(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """``HeadroomProxy.startup()`` passes ``rtk_wiring="disabled"`` through to
    ``configure_subscription_tracker``, ensuring RTK is disabled in proxy context.

    This test injects a synthetic ``headroom._core`` module so the proxy's
    transitive imports (content_router -> error_detection, tag_protector)
    can load without the compiled Rust extension.
    """
    import sys
    import types as _types

    # Build a complete synthetic _core module with all names imported at
    # module level by the proxy's transitive deps.
    _core_stub = _types.ModuleType("headroom._core")
    _core_stub.has_fast_tokenizer = lambda: False  # type: ignore[attr-defined]
    _core_stub.detect_language = lambda text: "unknown"  # type: ignore[attr-defined]
    _core_stub.count_code_tokens = lambda text, lang="": 0  # type: ignore[attr-defined]
    _core_stub.content_has_error_indicators = lambda text, context="": False  # type: ignore[attr-defined]
    _core_stub.keyword_registry_snapshot = lambda: {  # type: ignore[attr-defined]
        "error": [],
        "importance": [],
        "warning": [],
        "security": [],
    ...
    _core_stub.score_line = lambda line, ctx="": (None, 0.0, 0.0)  # type: ignore[attr-defined]
    ...
    original_configure = tracker_module.configure_subscription_tracker

    def spy_configure(**kwargs: Any) -> Any:
        captured_kwargs.update(kwargs)
        return original_configure(**kwargs)

    monkeypatch.setattr(tracker_module, "configure_subscription_tracker", spy_configure)
    ...
    config = ProxyConfig(
        ...
        subscription_poll_interval_s=3600,
        subscription_active_window_s=60,
    )
    ...
    assert captured_kwargs.get("rtk_wiring") == "disabled", (
        f"expected rtk_wiring='disabled', got {captured_kwargs.get('rtk_wiring')!r}"
    )
