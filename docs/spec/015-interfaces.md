...
| `--model-cost-map` | - | JSON object mapping model names to litellm cost params (env: `HEADROOM_MODEL_COST_MAP`) |
...
| `HEADROOM_MODEL_COST_MAP` | - | JSON object mapping model names to litellm cost params (overrides/injects into `litellm.model_cost`) |
